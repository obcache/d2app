...
GuiSetupTab(&ui,&cfg) {
	...
	if cfg.%toggleControl.name%Enabled 
		if !(fileExist(A_StartMenu "\Programs\Startup\d2app.lnk"))
			setAutoStart(1)
	else
		if (fileExist(A_StartMenu "\Programs\Startup\d2app.lnk"))
			setAutoStart(0)
	...
	setAutoStart(OnOff) {
		if (OnOff == "On" || OnOff == true || OnOff == 1) {
			try {
				fileCreateShortcut(installDir "/d2app.exe", A_StartMenu "\Programs\Startup\d2app.lnk",installDir,,"d2app Gaming Assistant",installDir "/img/attack_icon.ico")
			...
			try {
				fileDelete(A_StartMenu "\Programs\Startup\d2app.lnk")
				trayTip("d2app Autostart Disabled","d2app Config Change","Iconi Mute")
			} catch {
				trayTip("Failed to disable Autostart","d2app Config Change","Iconi Mute")
				setAutoStart(1)
			}
		}
	}
